...
CollocAgent is a **standalone application** for conservative collocation feedback in academic English. It requires no external service, hosted platform, or commercial model API, and runs entirely on a local machine. A hosted instance is also available at <https://collocagent.vercel.app> for readers who prefer to try it without installing anything. The artifact exposes the same pipeline through:
...
A hosted instance of the browser interface runs at <https://collocagent.vercel.app>.
It serves this repository unchanged: `api/index.py` is a small adapter that maps
the platform's request format onto the local HTTP service, and `vercel.json`
routes incoming requests to it. Nothing inside `collocagent/` is modified. The
hosted instance indexes the bundled synthetic corpus, so the counts it reports
demonstrate system behaviour and are not research evidence.

The artifact is not tied to that provider. `Dockerfile` and `docker-compose.yml`
build the same service for any container host, and `render.yaml` is a Render
blueprint for the same image. The service reads the port to bind from `PORT` and
answers a health check on `/health`.
